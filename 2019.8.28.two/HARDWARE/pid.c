...
void control(void)//通过增量式Pi算出pwm值
{
	...
	outPwm=outPwm+(errNow-errLast)*height_P+height_I*errNow+height_D*errlast1;
	if(outPwm>900)
	{
	  outPwm = 900;
	}
   ...
   switch(height)
	 {
		 ...
		 {
		  target = 130;//目标高度//A点
		  height_P = 1.3452;//pid的p系数
          height_I = 0.01258;//pid的i系数
		 }
		 break;
//		 case 2:
//		 {
...
//		 break;
		 ...
		 {
		  target = 200;//目标高度//C点
		  height_P = 0.8735;//pid的p系数
          height_I = 0.0135;//pid的i系数
		 }
		 break;
	     case 3:
		 {
